 ...
 *    unknown backdrop. `src/style.css` deliberately uses no `color-mix()` at
 *    all: every tone here is a literal hex on `:root`, which is the cheapest
 *    way to keep a contrast decision decidable. The SHARED TOP BAR is the
 *    exception and it is not this lab's to rewrite — `--cl-ink` is
 *    `color-mix(in srgb, var(--accent) 60%, #eafff8)`, its bottom border is a
 *    mix toward transparent, and both `.cl-btn` controls take their ink and
 *    their whole boundary from that token. axe files every one of those under
 *    `incomplete` rather than judging them, so a violations-only gate measured
 *    nothing about the header at all.
 *
 *  - TEXT FADED BY AN ANCESTOR'S `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This stylesheet has a rule
 *    against that: muted text lowers its LIGHTNESS (`--text-dim`,
 *    `--text-muted`) and never its opacity, and `.btn:disabled` recolours
 *    rather than fading. The one opacity in the file is the `panel-in`
 *    keyframe's `from { opacity: 0 }`, which is transient and cancelled
 *    outright under the reduced motion this gate asserts. The walk still
 *    models opacity, because that is a property of the current stylesheet and
 *    not of the page.
 *
 ...
 *    two real clippers and they both carry content: `.table-scroll`, which
 *    holds the key-schedule comparison, the S-box census, the semi-weak table,
 *    the MITM candidate list and every CAVP vector family; and `.strip`, the
 *    horizontally scrolling CBC block stream in the Sweet32 exhibit. Both are
 *    `role="region" tabindex="0"` so a keyboard can reach them. The
 ...
 *    failure for a table row scrolled out of view.
 *
 ...
 * verdict glyphs (✓ ✕ ! i) on `.verdict-glyph`, plus the `!` on the Sweet32
 * exhibit's permanent ARRANGED banner — each painted in a semantic ink
 * (`--good`, `--bad`, `--alarm`, `--warn`) beside words carrying the same
 * meaning, the states this lab exists to show — so `scan()` calls this a
 * second time as
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     * the CAVP vectors panel: six `<details>` families holding 299 table rows
     * between them, each row three 16-digit hex cells and two pills — plus the
     * 16-row key-schedule comparison the Same Circuit panel rebuilds on every
     * run. All of them are siblings re-walking the same ancestors up to
     * `<body>`. Without the caches the pass re-reads the same computed
     ...
     * that is `.table-scroll` (every table on the page) and `.strip` (the CBC
     * block stream), both of which really do scroll content out of view. The
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // of the shape: every learner check, the S-box census, the semi-weak
      // table, the NEG-1 scoping note and all six CAVP vector families are
      // <details> that ship shut. The
     ...
     *
     * Every `aria-hidden` element on this page is a glyph span emitted by
     * `src/ui/dom.ts`: the ✓ / ✕ / ! / i on each `.verdict-glyph`, and the `!`
     * on the Sweet32 exhibit's ARRANGED banner — plus the shared header's two
     * SVG marks, which carry no text. Each glyph duplicates the words directly
     * beside it, but each is painted in a SEMANTIC ink (`--good`, `--bad`,
     * `--alarm`, `--warn`), which is why `scan()` runs the whole `aria-hidden`
     * set through this walk with the exemption lifted rather than arguing any
     * of them is merely decorative.
     *
     * Nothing on this page hides a VALUE. No key, subkey, ciphertext, recovered
     * cookie or verdict text is inside an `aria-hidden` subtree — which was
     ...
     *
     * The one shape worth naming: the round diagram is an `<svg role="img">`
     * with an `aria-label` naming the halves it draws, so its inner `<text>`
     * runs are presentational to a reader — and still painted, so they are
     * still measured here. The same numbers are repeated as a `<dl>` beneath
     * it, which is the accessible source of truth.
     */
