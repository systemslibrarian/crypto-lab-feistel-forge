 ...
 * separates two tokens on purpose — `--border` is a DECORATIVE divider (card
 * edges, table rules) and is nowhere near 3:1, while `--control-edge` is the
 * boundary token and clears 3:1 against every surface in the palette. Tokens
 * are discarded by overrides though, and that is what this oracle exists to
 * measure rather than trust. The shapes worth naming: `.btn-primary` and the
 * selected `.tab-btn` both fill with `--accent` and so live or die by
 * fill-vs-surround rather than by an edge; every `.pill` variant repaints the
 * base `border: 1px solid transparent`; `input[aria-invalid="true"]` swaps its
 * edge to `--bad` and doubles its width, a state only the drive reaches; and
 * the shared top bar's two `.cl-btn` links have no fill at all, so their
 * `--cl-ink` mix IS the entire boundary. This oracle judges each control as
 * painted, at every driven state.
 *
 ...
 * disclosure triangles, which are `::marker`, the UA's own, and the custom
 * `<select>` chevron, which is a `background-image` gradient pair rather than a
 * pseudo-element. It runs anyway, at every state, so that stays a measurement
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * CAVP vectors panel: six `<details>` families holding 299 rows between
     * them, each with two pills, plus a summary pill per family — all
     * siblings re-walking the same ancestors up to `<body>`. Without the
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // uses exactly that as the `.pill` base rule, which every variant then
      // overrides with its own border-colour; counting the transparent spacer
      // as a border would judge a pill that had somehow lost its variant class
      // by an edge that paints nothing.
